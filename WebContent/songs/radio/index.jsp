...
<form name="searchForm" action="/isai/songs/radio/index.jsp" method="get" >
 ...
 <select  name="option" id="searchoption" onchange = "go()"  >
 ...
 <script>
  function go() {
	...
	//var x = document.getElementById("searchoption").value;
	 $.ajax({
	        type: "POST",
	        url: "/isai/auto?action=drop",
	        data: ({
	            variable : $(searchoption).val()
	        })
	    });
   ...
   jQuery( $(function() {
	...
	//  var getsearch= $('searchoption').val();
	//  var getsearch= $('#searchoption').val();
	 // alert(getsearch);
		$("#auto").autocomplete("/isai/auto?action=hid");
